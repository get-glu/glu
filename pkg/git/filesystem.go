...
func (f *filesystem) ReadDir(path string) (infos []os.FileInfo, err error) {
	f.logger.Debug("ReadDir", slog.String("path", path))
...
func (f *filesystem) MkdirAll(filename string, perm os.FileMode) error {
	logger := f.logger.With(slog.String("path", filename))
	logger.Debug("MkdirAll Started", slog.String("tree", f.tree.Hash.String()))
	defer func() {
		f.logger.Debug("MkdirAll Finished", slog.String("tree", f.tree.Hash.String()))
	}()
...
func updatePath(logger *slog.Logger, storage gitstorage.Storer, node *object.Tree, parts []string, insert bool, blob *plumbing.Hash) (err error) {
	defer func() {
		if err != nil {
